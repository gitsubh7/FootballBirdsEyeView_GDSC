...
def main(opt):
    ...
    if opt.save:
        ...
        output_name = output_name.split('.')[0] + '_newoutput.' + output_name.split('.')[-1]
    ...
    while(cap.isOpened()):
        ...
        ret, frame = cap.read()
        bg_img = gt_img.copy()
        # bg_img = cv2.resize(gt_img, (frame.shape[1], frame.shape[0]))
        ...
        if ret:
            ...
            frame[:, :] = cv2.resize(bg_img, (frame.shape[1], frame.shape[0]))
